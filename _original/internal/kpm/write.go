...
func PushManifest(ctx context.Context, pusher content.Pusher, id ID, layers []fs.FS, labels, annotations map[string]string) (ocispec.Descriptor, error) {
	var (
	...
	manifestDesc, manifestData, err := generateManifest(configDesc, id, annotations, layerDescs...)
	if err != nil {
...
func WriteImageManifest(w io.Writer, name reference.NamedTagged, id ID, layers []fs.FS, labels map[string]string, annotations map[string]string) (ocispec.Descriptor, error) {
	...
	manifestDesc, err := PushManifest(context.Background(), kpmStore, id, layers, labels, annotations)
	if err != nil {
...
func generateManifest(config ocispec.Descriptor, id ID, annotations map[string]string, layers ...ocispec.Descriptor) (ocispec.Descriptor, []byte, error) {
	idAnnotations := map[string]string{
		AnnotationName:    id.Name,
		AnnotationVersion: id.Version.String(),
		AnnotationRelease: id.Release,
	}
	manifestAnnotations := make(map[string]string, len(annotations)+len(idAnnotations))
	maps.Copy(manifestAnnotations, annotations)
	maps.Copy(manifestAnnotations, idAnnotations)
	...
	manifestDesc.Annotations = idAnnotations
