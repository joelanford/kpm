...
func validate(csv *operatorsv1alpha1.ClusterServiceVersion) error {
	...
	for _, r := range results {
		for _, w := range r.Warnings {
			// TODO: Move the validators to some other optional suite of validators.
			if strings.Contains(w.Detail, "should have an example annotation") ||
				strings.Contains(w.Detail, "example annotations not found") ||
				strings.Contains(w.Detail, "csv.Spec.minKubeVersion is not informed") {
				continue
			}
